public class Load3ds {
  private final int CHUNK_MAIN     = 0x4D4D;
	...
	private static final String LOG_TAG = Load3ds.class.getSimpleName();
	private AssetManager am;
	
	public Scene3D Load(Context context, String fileName)
	{
		am = context.getAssets();
		file = null;
		Scene3D scene = null; 
		if((iFile = getFile(fileName)) == null){
		...
		try {
			filePos = 0;
			file = new BufferedInputStream(iFile);
			scene = ProcessFile((long)iFile.available());
